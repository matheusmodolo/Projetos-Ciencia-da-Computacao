...
public class DFA {
     ...
     * destino não existirem e se já houver uma transição com o mesmo símbolo no
     * estado de origem.
     */
    ...
    /**
     * Verifica se uma string é aceita ou rejeitada pelo DFA, ou seja, se ela
     * pertence à linguagem definida por esse DFA.
     *
    ...
    public boolean accepts(String string) throws IllegalStateException {
        ...
        // implementação
        State atual = startingState;
        boolean achou;

        for (char c : string.toCharArray()) {
            achou = false;
            for (Transition t : atual.transitions) {
                if (t.symbol == c) {
                    atual = t.target;
                    achou = true;
                    break;
