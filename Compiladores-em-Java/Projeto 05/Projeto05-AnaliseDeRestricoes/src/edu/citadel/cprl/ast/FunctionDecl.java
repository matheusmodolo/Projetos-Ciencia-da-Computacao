...
public class FunctionDecl extends SubprogramDecl {
    ...
    public void checkConstraints() {
        ...
        try {
            List<InitialDecl> temp = getInitialDecls();
            List<ParameterDecl> temp2 = getFormalParams();

            for (InitialDecl initialDecl : temp) {
                initialDecl.checkConstraints();
            }

            for (ParameterDecl paramDecl : temp2) {
            ...
            List<Statement> stmt = stmtPart.getStatements();

            if (!hasReturnStmt(stmt)) {
