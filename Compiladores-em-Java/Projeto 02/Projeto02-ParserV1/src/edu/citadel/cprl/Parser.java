...
/**
 * Essa classe implementa um analisador sintático descendente recursivo para a
 * linguagem de programação CPRL.
 *
 * Versão corrente.
 *
 ...
 * examples\Incorrect\Arrays existem diversos arquivos de teste que contém o
 * código fonte de programas com alguma incorretude do ponto de vista léxico,
 ...
 * faça com que o parser analise corretamente o primeiro arquivo de teste,
 * depois o seguindo até o último (quinquagésimo nono).
 *
...
public class Parser {
    ...
    public void parseStatement() throws IOException {
        ...
        if (null == scanner.getSymbol()) {
            ...
            switch (scanner.getSymbol()) {
                case identifier:
                    if ((idTable.get(scanner.getToken()) != IdType.constantId)) {
                        if ((idTable.get(scanner.getToken()) != IdType.procedureId)) {
                            parseAssignmentStmt();
                        } else {
                        ...
                        try {
                            throw error("Identifier " + "\"" + scanner.getToken() + "\"" + " cannot start a statement.");
                        } catch (ParserException e) {
                            ErrorHandler.getInstance().reportError(e);
                            exit();
                        }
        ...
        try {
            ...
            if (scanner.getSymbol() == Symbol.notRW) {
                ...
                if (idType != null) {
                    ...
                    throw error("Identifier \"" + scanner.getToken() + "\" has not been declared.");
                }
    ...
    private void exit() throws IllegalStateException {
        ...
        /* Na versão final do ParserFC V1, a seção e código abaixo deve ser
